...
//    while ($error_no == 1062); //如果是订单号重复则重新提交数据
  ...
  $db->query("insert into order_genid (remark) values('9999')");
	...
	$xiangkuangnum=$db->getOne("select count(*) from ecs_order_info where from_unixtime(add_time,'%Y-%m-%d')='".$dt."' and (scts like '%相框%' or wsts like '%相框%') and (order_status=0 or order_status=1) ");
...
//    die(sprintf($_LANG['bonus_is_ok'], price_format($bonus['type_money'], false)));
    ...
    else
    {
       ...
       if (((!empty($bonus) && $bonus['user_id'] == $_SESSION['user_id']) || (($bonus['type_money'] > 0||($bonus['bonus_money'] > 0&&$bonus['flag']==1)) && empty($bonus['user_id']))) && $bonus['order_id'] <= 0)
        {
            //$order['bonus_kill'] = $bonus['type_money'];
            $now = gmtime();
			if( $bonus['use_end_date']>0)
			{
