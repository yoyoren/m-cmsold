...
if ($_REQUEST['act'] == 'order_list')
{
	...
	$smarty->assign('action_link', array('text' => '导出', 'href' => 'export_excel.php?adate='.$date.'&odate=""'));
...
function get_order_list()
{
	...
	$sql2="SELECT  b.order_sn, a.goods_name,a.goods_attr, a.goods_number,b.surplus,b.bonus,b.goods_amount,b.money_paid,b.order_amount, b.pay_name,b.pay_note,best_time FROM `ecs_order_goods` AS a LEFT JOIN ecs_order_info AS b ON a.order_id = b.order_id WHERE b.order_status =1 
				AND a.goods_price >=45 ".$where." ORDER BY surplus desc,bonus desc,pay_note desc ";
		$_SESSION['tcontent']=$GLOBALS['db']->getAll($sql2);	 	 	 	 		 	 	 	
		$_SESSION['ttop']=array("订单号","商品名称","商品磅重","商品数量","礼金卡","现金券","蛋糕折后金额","已支付款额","未支付款额","支付类型","备注","配送日期");
	$arr = array('item' => $row, 'filter' => $filter, 'page_count' => $filter['page_count'], 'record_count' => $filter['record_count']);
...
if($_REQUEST['act'] == 'cake_count')
{
	...
	if($date)
	{
		...
		foreach($order_list as $val)
		{
			...
			if($val['pay_note']=='销售活动'||$val['pay_note']=='销售赠送')
			{
				$xshd+=$val['goods_number'];
				$total_cake+=$val['goods_number'];
			}
			if($val['pay_note']=='公司赠送')
			{
				$gszs+=$val['goods_number'];
				$total_cake+=$val['goods_number'];
			}
		...
		$count=array("total_cake"=>	$total_cake,"ljk"=>$ljk,"cuxiao"=>$cuxiao,"normal"=>$normal,"yuejie"=>$yuejie,"xshd"=>$xshd,"schd"=>$schd,"gszs"=>$gszs,"scmini"=>$scmini);
	}
