...
if ($_REQUEST['act'] == 'maintain')
{
	...
	$smarty->assign('action_link', array('text' => '礼金卡生效', 'href' => 'moneycard1.php?act=affirm'));
...
elseif($_REQUEST['act'] == 'insert')
{
    ...
    for ($i = 0, $j = 0; $i < $type_sum; $i++)
    {
        ...
        $cardid = str_pad($cidxx, 11, '0', STR_PAD_LEFT);		
		$cardid="010".$y.$cardid; 
		$cardpassword = RandPassword(6,'ALL');
...
{	
    ...
    $smarty->assign('ur_here',     '礼金卡生效');
    $smarty->assign('action_link', array('text' => '礼金卡维护', 'href' => 'moneycard1.php?act=maintain'));
    $smarty->assign('admin_name',$_SESSION['admin_name']);    
    $smarty->assign('full_page',   1);
    $smarty->assign('admin_name',$_SESSION['admin_name']);
	$smarty->assign('lang',         $_LANG);
...
{	
  include_once(ROOT_PATH . 'includes/cls_json.php');
    $json = new JSON;
    $res=0;
    $mcs_id=$json->decode($_GET['mcs_id']);
    $sql="delete from moneycards where mcs_id=".$mcs_id." and flag=0";
    if($db->query($sql))
	{
		$res=1;
   		 $res = $json->encode($res);
...
elseif($_REQUEST['act']=='affirmaction')
{
    ...
    $use_enddate    = local_strtotime($_REQUEST['use_end_date'])+24*3600-1;
    ...
    sys_msg("礼金卡生效成功",0, $link);
}
/**
 * 已使用礼金卡查询
 */
elseif($_REQUEST['act']=='used_card')
{
	$smarty->assign('ur_here',     '已生效礼金卡使用情况查询');
    $smarty->assign('action_link', array('text' => '导出', 'href' => 'export_excel.php?odate=""&adate=""'));
    $smarty->assign('admin_name',$_SESSION['admin_name']);    
    $smarty->assign('full_page',   1);
	$smarty->assign('lang',         $_LANG);
	$scard=isset($_REQUEST['scard'])?$_REQUEST['scard']:'';
	$ecard=isset($_REQUEST['ecard'])?$_REQUEST['ecard']:'';
	$used_list=array();
	if($scard!="" && $ecard!="")
	{
		$sql="SELECT b.cardid,from_unixtime( b.used_time ) as usedtime1,cardmoney, a.rea_name, a.mobile_phone,a.home_phone, from_unixtime(b.sdate) as sdate1,from_unixtime(b.edate) as edate1 FROM `moneycards` AS b
				LEFT JOIN ecs_users AS a ON a.user_id = b.user_id WHERE mcs_id >= ".$scard." AND mcs_id <=".$ecard."";
		$used_list=$db->getAll($sql);
		foreach($used_list as $key=>$val)
		{
			if($val['usedtime1']=='1970-01-01 08:00:00')
			{
				$used_list[$key]['usedtime1']="";
			}
		}
		$_SESSION['tcontent']=$used_list;
		$_SESSION['ttop']=array("号段","充值时间","面值","姓名","手机","座机","生效日期","失效日期");
	}
	$smarty->assign("scard",$scard);
	$smarty->assign("ecard",$ecard);
	$smarty->assign("used_list",$used_list);
	$smarty->display("used_moneycards.html");
}
/**
 * 导出显示
 */
elseif($_REQUEST['act']=='gen_excel')
{
	...
	{   $row['id']=$i;$i++;
        ...
        if($row['user_id'])
        {
        	$sql="select user_name,mobile_phone from ".$ecs->table('users')." where user_id=".$row['user_id'];
        	$res_user=$db->getRow($sql);
			$row['user_name']=$res_user['user_name'];
			$row['mobile_phone']=$res_user['mobile_phone'];
        }else
		{
			$row['user_name']='';
			$row['mobile_phone']='';
        ...
        $type_list[]=$row;
		$_SESSION['tcontent'][]=array($row['mcs_id'],$type['mc_name'],$row['cardmoney'],$row['cardid'],$row['cardpassword'],$row['sdate'],$row['edate'],$row['flag1'],$row['aname'],$row['adate'],$row['user_name'],$row['mobile_phone'],$row['used_time']);
	}
		$_SESSION['ttop']=array("编号","礼金卡描述","面值","卡片序列号","卡片密码","使用起始日期","使用失效日期","状态","生效人","生效日期","使用会员","会员电话","使用时间");
...
else if($_REQUEST['act']=='account')
{
	...
	{   
	    ...
	    $res[$key]['format_user_money']=$val['user_money'];
	}
	//var_dump($res);
	$smarty->assign("type",$type);
...
function get_moneycards_list()
{
    ...
    $sql = "SELECT * FROM (SELECT ub.*, u.user_name,u.mobile_phone,bt.mc_name".
