...
strFrame+=' title="点击这里选择小时" align=center width=42>' ;
...
strFrame+=' onmouseover="style.backgroundColor=\'#aaccf3\'" onmouseout="style.backgroundColor=\'#63A3E9\'"';
strFrame+=' title="点击这里选择分钟" align=center width=42>' ;
strFrame+='					<span id=meizzMinuteHead></span></td>';
strFrame+='             <td style="cursor:hand" ';
strFrame+=' onmouseover="style.backgroundColor=\'#aaccf3\'" onmouseout="style.backgroundColor=\'#63A3E9\'"';
strFrame+='  align=center width=42>' ;
...
function tmpSelectHourInnerHTML(strHour) //小时的下拉框
{
	...
	var selectInnerHTML = s;
	var order=100;//此处应该是config.php中定义的一小时内最大订购量常量值
	for (var i = 10; i < 22; i++)
	{
...
function tmpSelectMinuteInnerHTML(strMinute) //分钟的下拉框
{
	...
	var selectInnerHTML = s;
	selectInnerHTML += "<option value='00'>请选择</option>\r\n";
	for (var i = 0; i < 4; i++)
	{
		if (i == m) { selectInnerHTML += "<option value='"+i*15+"' selected>"+i*15+"</option>\r\n"; }
		else { selectInnerHTML += "<option value='"+i*15+"'>"+i*15+"分</option>\r\n"; }
	}
	selectInnerHTML += "</select>";
...
function tmpSelectSecondInnerHTML(strSecond) //秒的下拉框
{
	...
	{
		if (i == m) { selectInnerHTML += "<option value='"+i*15+"' selected>"+i*15+"</option>\r\n"; }
		else { selectInnerHTML += "<option value='"+i*15+"'>"+i*15+"秒</option>\r\n"; }
	}
	selectInnerHTML += "</select>";
	...
	if(outObject)
	{
		if (bUseTime)
		{
			outObject.value= parent.meizzTheYear + "-" + format( parent.meizzTheMonth) + "-" + 
							...
							format(parent.meizzTheMinute) + ":00"; 
							//注：在这里你可以输出改成你想要的格式
		}
		else
		{
...
//		outObject.value= yy + "-" + format(mm) + "-" + format(n) + " " + format(hh) + ":" + format(mi) + ":" + format(se); //注：在这里你可以输出改成你想要的格式
		if (hh>21 || hh<10)	//小时不能在22点到10点之间
		{
			hh=14;
		}
		mi=Math.floor(mi/15)*15;		//分钟
