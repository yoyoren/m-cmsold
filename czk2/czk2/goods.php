 ...
 * ============================================================================
 * $Author: testyang $
 * $Id: goods.php 14691 2008-06-30 05:26:53Z testyang $
*/

define('IN_ECS', true);

require(dirname(__FILE__) . '/includes/init.php');
require_once(ROOT_PATH . '/sys/includes/lib_goods.php');
include_once(ROOT_PATH . '/sys/includes/cls_image.php');
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
	...
	/*同步论坛版块*/
	//$typename=$db->getOne("select cat_name from ecs_category where cat_id='$catgory_id'");
	
	//$typename="蛋糕";//empty($typename) ? "蛋糕" : $typename;
	//include ROOT_PATH."data/lyconfig.php";
	//$dgid=$disdb->getRow("select fid,type from cdb_forums where name='$typename'");
	//die("insert into cdb_forums(fup,type,name,status) values('$dgid','forum','$_POST[goods_name]','1')");
	//$dgid["type"]=$dgid["type"]=='group' ? 'forum' : 'sub';
	//$disdb->query("insert into cdb_forums(fup,type,name,status) values('".$dgid["fid"]."','".$dgid["type"]."','$_POST[goods_name]','1')");
    ...
    else
    {
        ...
        $sql = "UPDATE " . $ecs->table('goods') . " SET " .
                ...
                "goods_name_style = '$_POST[goods_name_style]', " .
                "goods_sn = '$goods_sn', " .
                "cat_id = '$catgory_id', " .
                "brand_id = '$brand_id', " .
