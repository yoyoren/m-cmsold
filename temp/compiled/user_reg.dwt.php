					...
					<tr>
						...
						<td rowspan='2'>
							<table style="margin-left:280px;visibility:hidden;" border="0" >
								<tr>
									<td><input type="button" name="Submit" class="bt"  value="&nbsp;"  style="width:120px;"/></td>
									<td>&nbsp;&nbsp;&nbsp;</td>
						...
						<tr><td>&nbsp;</td></tr>
						<tr>
							<td>&nbsp;</td><td colspan='2' style='padding-left:0px;'><input type="button" name="Submit" class="bt"  onclick='register2()' value="&nbsp;"  style="width:174px;height:30px;cursor:pointer;border:0;"/></td>
						</tr>
						<tr>
...
function register2()
{
	var mobile_phone=document.formUser.mobile_phone.value;
	var password=document.formUser.password.value;
	var password2=document.formUser.confirm_password.value;
	...
	{
		document.getElementById('conform_password_notice').innerHTML = "两次输入的密码不一致";return false;
	}
	...
	if(mobile_phone.length>0)
	{
		 $.ajax({
              type:"post",
              url:"check.php",
              data:"id="+new Date()+"&mobile="+mobile_phone,
              dataType:"html",
              success:function(msg){
                  //接收服务器的响应结果
				 if(msg==1){
                  $("#mobile_notice").html('改手机号已注册，请换个手机号');
				  return false;}
				  else
				  {
