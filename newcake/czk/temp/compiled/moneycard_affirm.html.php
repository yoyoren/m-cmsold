...
function validate()
{
 ...
 if(parseInt(scardnum.value) > parseInt(scardid.value)){msg=msg+"生效的起始编号要大于等于编号范围最小值"+"\n";};
  ...
  if(!sdate || !edate){msg=msg+"请选择储值卡使用起始日期和失效日期";}
  if(edate<sdate){msg=msg+"储值卡使用失效日期必须大于等于起始日期";}
...
function check_cardid(result){
	...
	if(result>0){alert("此范围内的部分储值卡已生效，请重新输入生效范围！");}
